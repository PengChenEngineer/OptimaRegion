...
n <- 20
sd_noise <- 0.05
chain_length <- 10000 # ~ mcmc speed
thin_interval <- 10 # ~ theta2opt speed
grid_size <- 100
n_path <- 200
```
...
```{r}
x_grid <- seq(min(design), max(design), length.out = grid_size) %>% as.matrix(nrow = grid_size)
```


<center>
```{r echo=FALSE, fig.height=3, fig.width=6, out.extra='style="border-color:white"'}
tibble(design = design, y = y) %>%
  ggplot(aes(x = design, y = y)) + 
  geom_point() + 
...
gp_model <- "model{
  ...
  # prior
  process_mean ~ dunif(-3, 3)
  length_scale ~ dunif(0, 10)
  fun_var ~ dunif(0, 2)
  noise_var ~ dunif(0, 1)
}"
# compile
gp_jags <- jags.model(
  ...
  data = list(y = y, d = fields::rdist(design)),
...
gp_jags <- jags.model(
  ...
  data = list(
    ...
    fun_var_lb = 0, fun_var_ub = 2,
    noise_var_lb = 0, noise_var_ub = 1
  ),
  inits = list(.RNG.name = "base::Wichmann-Hill", .RNG.seed = 100),
  quiet = TRUE
)
...
```{r include=FALSE}
plot_chain <- function(chain) {
  traces <- purrr::imap(
  chain,
  ~ ggplot(chain, aes(x = 1:length(.x), y = .x)) + 
  geom_line() + 
...
summary_chain <- function(chain)
  purrr::map(chain, ~ quantile(x = .x, probs = c(0.025, 0.25, 0.5, 0.75, 0.975))) %>%
  unlist() %>%
  matrix(nrow = 4, byrow = TRUE) %>%
  `rownames<-`(names(chain)) %>%
...
```{r eval=FALSE, include=FALSE}
summary_chain(gp_chain)
```


```{r eval=FALSE, include=FALSE}
# 2.1.2  Bayesian Kriging
gp_cond <- function(x, design, y, process_mean, length_scale, fun_var) {
  # see STAT 597 slides
  C <- apply(
    X = fields::rdist(design), MARGIN = c(1, 2),
    FUN = function(d) fun_var * exp(-d / length_scale)
  )
  c <- apply(
    X = fields::rdist(x1 = design, x2 = x), MARGIN = c(1, 2),
    FUN = function(d) fun_var * exp(-d / length_scale)
  )
  cond_mean <- process_mean + crossprod(c, solve(C, y - rep(process_mean, length(y))))
  cond_var <- fun_var - crossprod(c, solve(C, c)) %>% as.numeric()
  list(cond_mean = as.numeric(cond_mean), cond_sd = sqrt(cond_var))
}
```

```{r eval=FALSE, include=FALSE}
gp_cond(
  x = 1.5332628, design = design, y = y, 
  process_mean = gp_chain$process_mean[1],
  length_scale = gp_chain$length_scale[1], 
  fun_var = gp_chain$fun_var[1]
)
```

#### 2.1.2 Credible Region

```{r eval=FALSE, include=FALSE}
# Six-Sigma Bound Method
x_grid <- seq(0, 5, length.out = 1e2)
theta_mean2sim <- purrr::map(
  .x = x_grid,
  .f = ~ gp_cond(
    x = .x, design = design, y = y,
    process_mean = mean(gp_chain$process_mean),
    length_scale = mean(gp_chain$length_scale),
    fun_var = mean(gp_chain$fun_var)
  )
) %>%
  map_df(
    .f = ~ tibble(
...
tibble(design = design, y = y) %>%
  ggplot(aes(x = design, y = y)) +
  geom_point() +
  stat_function(fun = function(i) flex_1(i)) +
  geom_ribbon(
    data = theta_mean2sim,
    aes(
      x = x_grid,
  ...
  geom_line(data = theta_mean2sim, aes(x = x_grid, y = mean + 3 * sigma), col = "red") +
  geom_line(data = theta_mean2sim, aes(x = x_grid, y = mean), col = "red", alpha = 0.5) + 
  xlab("x") +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
```
...
```{r eval=FALSE, include=FALSE}
x_grid <- seq(0, 5, length.out = 1e2)
theta2krig_plot <- function(process_mean, length_scale, fun_var) {
  purrr::map(
    .x = x_grid,
    .f = ~ gp_cond(
      x = .x, design = design, y = y,
...
gp_bands <- purrr::pmap(
  .l = sample_n(gp_chain[, 1:3], 10),
  .f = function(process_mean, length_scale, fun_var)
    theta2krig_plot(process_mean, length_scale, fun_var)
) %>%
  purrr::imap(
    .f = ~ tibble(
      mean = .x$mean,
...
ggplot(
  ...
  geom_line() + 
  stat_function(fun = function(i) flex_1(i), col = "black") +
  xlab("x") + ylab("y") + 
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
```

#### Map from $\theta^{[t]}$ to $\{x^{*[t](s)}\}$

```{r}
theta2opt<- function(n_path, x_grid, design, y, process_mean, length_scale, fun_var) {
  ...
  paths <- MASS::mvrnorm(n = n_path, mu = mu, Sigma = Sigma) %>%
    t() %>%
    as.data.frame() %>%
    set_names(map_chr(1:n_path, ~ paste("path_", .x, sep = "")))
  x_grid[apply(paths, 2, which.max)]
}
```

```{r eval=FALSE, include=FALSE}
optima <- theta2opt(
  n_path = 6, x_grid = x_grid, design = design, y = y, 
  process_mean = mean(gp_chain$process_mean),
  length_scale = mean(gp_chain$length_scale),
  fun_var = mean(gp_chain$fun_var)
)
...
```{r eval=FALSE, include=FALSE}
con_sims_plot <- tibble(
  x = rep(x_grid, length(cond_sims)),
  y = unlist(cond_sims),
  realization = purrr:::map(
    .x = 1:length(cond_sims),
    .f = ~ rep(.x, nrow(x_grid))
  ) %>%
    unlist() %>%
    factor()
...
cond_opts <- tibble(
  opts = x_grid[apply(cond_sims, 2, which.max)],
  realization = factor(1:num_realization)
)
ggplot(data = NULL, aes(x = design, y = y)) +
  geom_point() +
  geom_line(
    data = con_sims_plot,
    aes(
  ...
  geom_vline(
    data = cond_opts,
    aes(xintercept = opts, col = realization),
    alpha = 0.5
  ) + 
  geom_point(data = NULL, aes(x = design, y = y)) + 
  stat_function(fun = flex_1, col = "black") +
...
```{r}
thetas2opt <- function(thetas, n_path, x_grid, design, y) {
  lapply(
    X = 1:nrow(thetas),
    FUN = function(i) theta2opt(
        n_path = n_path, x_grid = x_grid, design = design, y = y,
...
```{r eval=FALSE, include=FALSE}
res <- thetas2opt(thetas = gp_chain, n_path = 6, x_grid = x_grid, design = design, y = y)
```

#### Parallel Implementation

```{r}
thetas2opt_par <- function(thetas, n_path, x_grid, design, y,
                                  process_mean, length_scale, fun_var) {
  cl <- makeCluster(detectCores(logical = FALSE) - 1)
  clusterEvalQ(cl, {
    library(tidyverse)
  })
  clusterExport(cl, c("theta2opt", "x_grid", "design", "y", "n_path"))
  res <- clusterApply(
    cl, 
    x = 1:nrow(thetas),
    fun = function(i) theta2opt(
...
```{r eval=FALSE, include=FALSE}
res <- thetas2opt_par(
  thetas = gp_chain, n_path = n_path, x_grid = x_grid, design = design, y = y
)
```
...
microbenchmark(
  res = thetas2opt(
    thetas = gp_chain, n_path = 200, x_grid = x_grid, design = design, y = y
  ),
  res_par = thetas2opt_par(
    thetas = gp_chain, n_path = 200, x_grid = x_grid, design = design, y = y
  ),
  times = 1
)
...
```{r}
BayesOptRegionGP1 <- function(design, y, process_mean_lb, process_mean_ub,
                              length_scale_lb, length_scale_ub,
                              fun_var_lb, fun_var_ub, noise_var_lb, noise_var_ub,
                              chain_length, thin_interval,
                              path_grid, path_per_theta,
                              parallel = TRUE) {
  ...
  # map thetas to sample paths to optima
  if (parallel) {
    optima <- thetas2opt_par(
      thetas = thetas, n_path = path_per_theta, x_grid = path_grid, design = design, y = y
    )
  } else{
    optima <- thetas2opt(
      thetas = thetas, n_path = path_per_theta, x_grid = path_grid, design = design, y = y
  ...
  list(
    ...
    optima = structure(optima, class = "baysoptima1D")
  )
}
```
...
```{r include=FALSE}
plot.baysoptima1D <- function(optima){
...
```{r include=FALSE}
summary.baysoptima1D <- function(optima) {
...
```{r}
c(thetas, optima) %<-% BayesOptRegionGP1(
  design = design, y = y, process_mean_lb = -3, process_mean_ub = 3,
  length_scale_lb = 0, length_scale_ub = 10,
  fun_var_lb = 0, fun_var_ub = 2, noise_var_lb = 0, noise_var_ub = 1,
  chain_length = chain_length, thin_interval = thin_interval,
  path_grid = x_grid, path_per_theta = 200
)
```
