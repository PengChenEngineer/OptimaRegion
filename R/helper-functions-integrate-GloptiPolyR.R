...
Gloptipolym <- function(coefficients, k, degree, lb, ub, maximization){
  ...
  # Objective function ------------------------------------------------------
  P<- list(); c <- array(0, dim = rep(degree + 1, k))
  ...
  for(i in 1:nrow(id)){
    eval(parse(text = paste("c[", toString(id[i,]),
                            "] <- coefficients[", i, "]")))
  ...
  for(i in 1:k){ # loop through variables
    ...
    index_for_c <- rep(1, k); index_for_c[i] <- 2
  ...
  # Call GloptiPolyR --------------------------------------------------------
  res <- GloptiPolyR(Ps); solution <- res$solution
  if(maximization){ # assume GloptiPolyR only supports "min"
